import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css                   */const i=[{id:1,nombre:"Botella de Agua Reutilizable",precio:15e3,imagen:"https://via.placeholder.com/200x200/22c55e/ffffff?text=Botella+Eco",huellaCarbonoKg:-2.3,puntosVerdes:15,categoria:"Sustentable",descripcion:"Botella de acero inoxidable, libre de BPA"},{id:2,nombre:"Bolsa Ecológica de Tela",precio:8e3,imagen:"https://via.placeholder.com/200x200/16a34a/ffffff?text=Bolsa+Eco",huellaCarbonoKg:-1.2,puntosVerdes:10,categoria:"Sustentable",descripcion:"Bolsa reutilizable de algodón orgánico"},{id:3,nombre:"Panel Solar Portátil",precio:85e3,imagen:"https://via.placeholder.com/200x200/eab308/ffffff?text=Panel+Solar",huellaCarbonoKg:-15.8,puntosVerdes:50,categoria:"Energía Renovable",descripcion:"Panel solar de 20W para dispositivos móviles"},{id:4,nombre:"Compost Orgánico 5kg",precio:12e3,imagen:"https://via.placeholder.com/200x200/84cc16/ffffff?text=Compost",huellaCarbonoKg:-3.5,puntosVerdes:12,categoria:"Jardinería",descripcion:"Compost 100% orgánico para plantas"},{id:5,nombre:"Lámpara LED Solar",precio:25e3,imagen:"https://via.placeholder.com/200x200/f59e0b/ffffff?text=Lampara+LED",huellaCarbonoKg:-8.2,puntosVerdes:25,categoria:"Energía Renovable",descripcion:"Lámpara con panel solar integrado"},{id:6,nombre:"Kit de Reciclaje",precio:18e3,imagen:"https://via.placeholder.com/200x200/059669/ffffff?text=Kit+Reciclaje",huellaCarbonoKg:-4.1,puntosVerdes:20,categoria:"Reciclaje",descripcion:"Kit completo para separar residuos en casa"}],d=[{id:1,numero:"**** **** **** 1234",tipo:"Visa",nombre:"Juan Pérez",vencimiento:"12/27"},{id:2,numero:"**** **** **** 5678",tipo:"Mastercard",nombre:"Juan Pérez",vencimiento:"03/26"}];let o=null,r=parseInt(localStorage.getItem("puntosUsuario"))||180;const c=[{id:1,nombre:"5% Descuento",puntos:10,activo:!0,app:"EcoPuntos Verdes"},{id:2,nombre:"10% Walmart",puntos:25,activo:!0,app:"Walmart"},{id:3,nombre:"Envío Gratis",puntos:15,activo:!1,app:"EcoPuntos Verdes"}];document.addEventListener("DOMContentLoaded",()=>{m(),l();const t=document.getElementById("btnCanjearPuntos");t&&t.addEventListener("click",s);const e=document.getElementById("linkCupones");e&&e.addEventListener("click",a=>{a.preventDefault(),s()})});function m(){const t=document.getElementById("productosGrid");t.innerHTML=i.map(e=>`
    ...
    `).join("")}function u(t){return`${Math.abs(t/22).toFixed(1)} árboles plantados`}window.comprarProducto=function(t){o=i.find(e=>e.id===t),p()};function p(){const t=document.getElementById("modalPago"),e=document.getElementById("detalleProducto"),a=document.getElementById("tarjetasDisponibles");e.innerHTML=`
    ...
    `).join(""),t.classList.remove("hidden")}window.seleccionarTarjeta=function(t){d.find(e=>e.id===t),document.querySelectorAll(".tarjeta-opcion").forEach(e=>e.classList.remove("border-blue-500","bg-blue-50")),event.currentTarget.classList.add("border-blue-500","bg-blue-50"),document.getElementById("btnPagar").disabled=!1};window.procesarPago=function(){const t=document.getElementById("btnPagar");t.innerHTML='<i class="fas fa-spinner fa-spin mr-2"></i>Procesando...',t.disabled=!0,setTimeout(()=>{r+=o.puntosVerdes,localStorage.setItem("puntosUsuario",r),g(),b(),cerrarModal()},2e3)};function g(){const t=JSON.parse(localStorage.getItem("historialCompras"))||[];t.push({id:Date.now(),fecha:new Date().toISOString().split("T")[0],producto:o.nombre,precio:o.precio,puntos:o.puntosVerdes,carbono:o.huellaCarbonoKg,categoria:o.categoria}),localStorage.setItem("historialCompras",JSON.stringify(t))}function b(){const t=document.getElementById("modalConfirmacion"),e=document.getElementById("resumenCompra"),a=document.getElementById("impactoAmbiental");e.innerHTML=`
    ...
    `,a.innerHTML=`
        <div class="text-center">
            ...
            <div class="text-sm text-gray-600">${u(o.huellaCarbonoKg)}</div>
        </div>
    `,l(),t.classList.remove("hidden")}function l(){document.getElementById("puntosUsuario").textContent=`${r} Puntos Verdes`}window.cerrarModal=function(){document.getElementById("modalPago").classList.add("hidden"),o=null,document.getElementById("btnPagar").innerHTML="Pagar Ahora",document.getElementById("btnPagar").disabled=!0};window.cerrarConfirmacion=function(){document.getElementById("modalConfirmacion").classList.add("hidden")};function s(){document.getElementById("modalCupones").classList.remove("hidden"),f()}function f(){const t=document.getElementById("listaCupones");t.innerHTML="",c.forEach(e=>{const a=r<e.puntos||!e.activo?"disabled opacity-50 cursor-not-allowed":"",n=document.createElement("div");n.className="flex justify-between items-center p-3 bg-gray-50 rounded-lg",n.innerHTML=`
            <div>
                <div class="font-semibold">${e.nombre}</div>
                <div class="text-xs text-gray-500">${e.puntos} puntos - ${e.app}</div>
            </div>
            <button class="px-3 py-1 bg-green-500 text-white rounded-lg hover:bg-green-600 ${a}" 
                ${a?"disabled":""} 
                onclick="canjearCupon(${e.id})">
                Canjear
            </button>
        `,t.appendChild(n)})}window.cerrarModal=function(){document.getElementById("modalPago").classList.add("hidden"),o=null,document.getElementById("btnPagar").innerHTML="Pagar Ahora",document.getElementById("btnPagar").disabled=!0};window.cerrarConfirmacion=function(){document.getElementById("modalConfirmacion").classList.add("hidden")};window.cerrarCupones=function(){document.getElementById("modalCupones").classList.add("hidden")};function v(t){const e=document.getElementById(t);e.addEventListener("click",a=>{a.target===e&&e.classList.add("hidden")})}["modalPago","modalConfirmacion","modalCupones"].forEach(t=>v(t));
